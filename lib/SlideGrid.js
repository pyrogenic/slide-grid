    ...
    function SlideGrid(props) {
        ...
        _this.tick = function () {
            ...
            if (active && _this.lastInputEvent.touchCount !== undefined && location && (Date.now() - location.timestamp) > 300) {
                ...
                if (!isDragging && _this.props.canExchange(active.id)) {
        ...
        _this.onMouseOrTouchDown = function (event) {
            ...
            if (target) {
                ...
                setTimeout(function () {
                    if (_this.state.active === target && !target.classList.contains("dragging") && _this.props.canExchange(target.id)) {
        ...
        _this.onMouseOrTouchMove = function (event) {
            ...
            if (canDrag) {
                ...
                var d2 = dx * dx + dy * dy;
                if (!isDragging && d2 > 9) {
                    if (_this.props.canExchange(active.id)) {
                ...
                if (isDragging) {
                    ...
                    if (target) {
                        ...
                        if (target !== active && !sliding && _this.props.canExchange(target.id, active.id)) {
        ...
        _this.onMouseOrTouchUp = function (event) {
            ...
            _this.setState({ active: undefined, location: undefined, wiggle: false }, function () {
                if (click) {
                    _this.props.tap(click);
                }
                else {
                    _this.props.done(done);
                }
            });
        };
        ...
        _this.state = {};
        _this.uniqueId = "slide-component-" + ++SLIDE_GRID_INSTANCE_ID;
        return _this;
    }
    SlideGrid.prototype.render = function () {
        return React.createElement("div", { id: this.uniqueId, className: this.props.className + " " + (this.state.wiggle ? "wiggle" : ""), onMouseDown: this.onMouseDown, onMouseMove: this.onMouseMove, onMouseUp: this.onMouseUp }, this.children);
    };
