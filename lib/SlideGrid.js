            ...
            if (canDrag) {
                ...
                if (isDragging) {
                    ...
                    if (target) {
                        ...
                        var path = _this.graph.path(active.id, target.id);
                        if (!path) {
                            return;
                        }
                        ...
                        var exchangeTarget_1 = document.getElementById(path.shift());
                        // const sliding = exchangeTarget && exchangeTarget.classList.contains(SLIDING);
                        if (exchangeTarget_1) {
                            var er = exchangeTarget_1.getBoundingClientRect();
                            var emptyLeft = emptyLocation.left;
                            var emptyTop = emptyLocation.top;
                            var sdx = emptyLeft - er.left;
                            var sdy = emptyTop - er.top;
                            exchangeTarget_1.classList.add(SLIDING);
                            exchangeTarget_1.style.transform = "translate(" + sdx + "px," + sdy + "px)";
                            exchangeTarget_1.style.transition = "all " + _this.tuning.slideDurationMS + "ms ease-in-out";
                            var a_1 = active.id;
                            var b_1 = exchangeTarget_1.id;
                            setTimeout(function () {
                                exchangeTarget_1.classList.remove(SLIDING);
                                exchangeTarget_1.style.transform = null;
                                exchangeTarget_1.style.transition = "";
