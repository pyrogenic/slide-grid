...
var SlideGrid = /** @class */ (function (_super) {
    ...
    function SlideGrid(props) {
        ...
        _this.getTarget = function (event) {
            ...
            var x = event.clientX;
            var y = event.clientY;
            if (target && (sameElement(_this.active, target) || !elementContainsPoint(target, x, y))) {
                var insideActive = false;
                var otherTarget = _this.childElements.find(function (element) {
                    var inside = elementContainsPoint(element, x, y);
                    if (element === target) {
                        insideActive = true;
                        return false;
                    }
        ...
        _this.onMouseOrTouchDown = function (event) {
            ...
            if (target) {
                ...
                var parentX = target.parentElement.offsetLeft;
                var parentY = target.parentElement.offsetTop;
                var emptyLocation = {
                    left: rect.left,
                    top: rect.top,
                    // parentX,
                ...
                setTimeout(function () {
                    ...
                    if (sameElement(active, target) && !target.classList.contains(DRAGGING) && _this.canExchange(target.id) !== false) {
        ...
        _this.onMouseOrTouchMove = function (event, onlyUpdateActive) {
            ...
            var active = _this.active;
            var _a = _this.state, emptyLocation = _a.emptyLocation, activeLocation = _a.location;
        ...
        _this.onMouseOrTouchUp = function (event) {
            ...
            if (active) {
                ...
                else if ((target === null || target === void 0 ? void 0 : target.id) === active.id && state.location) {
        ...
        _this.recordTouch = function (kind, event) {
            ...
            else if (_this.lastInputEvent) {
                clientX = _this.lastInputEvent.clientX;
                clientY = _this.lastInputEvent.clientY;
            }
    ...
    SlideGrid.prototype.render = function () {
        ...
        //     }, 1);
        // }
        return React.createElement("div", { id: this.uniqueId, className: compact([SLIDE_GRID, this.props.className, this.state.wiggle && WIGGLE]).join(" "), style: this.props.style, onMouseDown: this.onMouseDown, onMouseMove: this.onMouseMove, onMouseUp: this.onMouseUp }, this.props.children);
    };
